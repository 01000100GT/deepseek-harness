...
**`dsh-web-fetch-http`** 保持不挂，`dsh-tool-web` 保持 `fetch: false`。提供方只允许连接到已验证的公开 IP 目的地址，但 `dsh-tool-web` 没有 web 专用权限策略，执行时也不会询问 `ctx.approval`（[README](../../../../packages/web/tool-web/README.zh.md)）。因此，已交付的权限 preset 不会从受 sandbox 约束的文件访问静默扩展到模型选择的公开网络请求。
