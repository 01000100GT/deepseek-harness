...
- **通过 `ctx.connection` 暴露上传并沿 Session 构造器传递该对象。** 拒绝：Connection 不拥有文件传输，中间对象层也不使用这项依赖。独立 Cordis 服务由 Session 现有的 Agent scope 继承，并允许普通浏览器与 Worker-hosted 页面替换载体，而无需扩大 Connection。
