...
该组合包拥有一个 DeepSeek 适配器、SDK JSON-RPC 服务、无执行器的 agent 主干、本地子进程与不受限文件系统提供方、持久 Bash、字符串替换 editor，以及位于 `$DSH_HOME/sessions` 的未压缩 JSONL 会话。Persona 来自 `DSH_SYSTEM_PROMPT`，适配器路由来自 `DSH_MODEL` 与 `DSH_CONTEXT_WINDOW`，凭据来自 `DEEPSEEK_API_KEY`。
