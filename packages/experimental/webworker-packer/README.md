...
- **Reachability requires a statically named request** — computed `import`, `require`, and `createRequire` arguments resolve only at runtime and fail loud if the target was otherwise pruned; a runtime-selected module needs an explicit image entry seed.
