...
3. **可达性 sweep**——用运行时加载器自己的解析，从全部 workspace 导出面加 worker 装配种子（`IMAGE_ENTRY_SEEDS`）出发，pack 时把每个可达模块降低到包装契约。Transform 会报告具名静态 import、re-export、动态 import，以及经 `require` 或导入的 `createRequire` 发起的调用。页面资产（`./client` 导出背后的 `lib/client.js`）原样直发；自家代码的不可解析请求打包即失败，第三方的容忍到 require 时 fail loud。
...
- **可达性要求请求名称静态可知**——计算得到的 `import`、`require` 与 `createRequire` 参数只在运行时解析；若目标已被裁掉就会立即失败。运行时选择的模块需要显式镜像入口种子。
