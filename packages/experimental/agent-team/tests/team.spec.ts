...
describe('Team mailbox and waiting', () => {
  ...
  it('cold-resumes an inactive sibling with sender attribution', async () => {
    ...
    const peerMessages = stored.events.flatMap(event => event.type === 'user/message'
      && event.data.source.kind === 'team-message' ? [event.data] : [])
    expect(peerMessages.map(message => message.content.at(-1))).toEqual([
      { type: 'text', text: 'first update' },
    ])
    expect(peerMessages.map(message => message.content[0]?.type === 'text' && message.content[0].text)).toEqual([
      expect.stringMatching(/^Team message .* from alpha:$/u),
    ])
    expect(peerMessages.map(message => message.source.kind === 'team-message'
      ? [message.source.messageId, message.source.senderName]
      : undefined)).toEqual([
