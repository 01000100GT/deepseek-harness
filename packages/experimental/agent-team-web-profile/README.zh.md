...
Agent Teams 的私有 Web profile 层。应当在 `@deepseek-ai/dsh-web-app` 与 [`@deepseek-ai/dsh-agent-team-profile`](../agent-team-profile/README.md) 之后应用。本 patch 先插入实验性 Host Remote adapter，再插入 Team 会话页头 UI；它不修改稳定 Web bundle。
...
Host profile 提供 Team domain 与模型工具。本 Web 层提供浏览器 adapter、生成式 Client Remote namespace 与呈现。移除任一实验性 bundle 后，稳定 base 与 Web composition 保持不变。
