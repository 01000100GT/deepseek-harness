...
describe('TeamAction', () => {
  ...
  it('keeps a successful task mutation newer than an in-flight refresh', async () => {
    ...
    const updateTask = vi.fn(() => Promise.resolve({
      ok: true as const,
      value: { ...task, revision: 2, status: 'completed' as const },
    }))
  ...
  it('shows load and create failures and ignores a create result after a session switch', async () => {
    const failedLoad = actions({
      load: () => Promise.resolve({ ok: false, error: 'load failed', conflict: false }),
    })
    ...
    const createTask = vi.fn(() => Promise.resolve({
      ok: false as const, error: 'create failed', conflict: false,
    }))
  ...
  it('contains stale-session and ordinary task failures without retrying', async () => {
    ...
    pending.resolve({ ok: true, value: { ...task, revision: 2, status: 'completed' } })
    await Promise.resolve()
    expect(screen.queryByText('Implement runtime')).toBeNull()
    rendered.unmount()

    render(<TeamAction {...props(actions({
      updateTask: () => Promise.resolve({ ok: false, error: 'update failed', conflict: false }),
    }))} />)
    fireEvent.click(screen.getByRole('button', { name: /Agent Team/u }))
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /完成/u }))
  ...
  it('contains edit and dependency failures and supports form cancellation and unassignment', async () => {
    const updateTask = vi.fn()
      .mockResolvedValueOnce({ ok: false, error: 'edit failed', conflict: false })
      .mockResolvedValueOnce({ ok: true, value: { ...task, revision: 2, subject: 'Saved edit' } })
      .mockResolvedValueOnce({ ok: false, error: 'dependency failed', conflict: false })
      .mockResolvedValueOnce({ ok: true, value: { ...task, revision: 2, ownerName: undefined } })
    render(<TeamAction {...props(actions({ updateTask }))} />)
    fireEvent.click(screen.getByRole('button', { name: /Agent Team/u }))
    await screen.findByText('Implement runtime')
  ...
  it('reloads a dependency conflict and ignores dependency settlement after a session switch', async () => {
    ...
    const conflictUpdate = vi.fn()
      .mockResolvedValueOnce({ ok: true, value: { ...task, revision: 2, subject: 'Conflict edit' } })
      .mockResolvedValueOnce({ ok: false, error: 'stale dependency', conflict: true })
    ...
    const staleUpdate = vi.fn()
      .mockResolvedValueOnce({ ok: true, value: { ...task, revision: 2, subject: 'Late edit' } })
      .mockResolvedValueOnce({ ok: false, error: 'stale dependency', conflict: true })
    ...
    const dependency = Promise.withResolvers<TeamActionResult<TeamTask>>()
    ...
    third.rerender(<TeamAction {...props(actions(), 'next-session' as SessionId)} />)
    dependency.resolve({ ok: true, value: { ...task, revision: 3, subject: 'Late dependency' } })
    await Promise.resolve()
