...
- `ProjectionDefinition<K, S>` — `{ key, stateSchema, init(header), apply(state, event), wire?, stateVersion }`: a synchronous state-driven computation unit. `init` receives the immutable `SessionHeader`; `wire` supplies `viewSchema` and `view`, and omitting it makes the unit host-only.

## Contract

- **The framework drives, the domain computes.** The registry subscribes to `session/event` once; every committed event passes every unit's `apply` eagerly. Domains hold no subscriptions. Cells (`{state, observedSeq}` per unit per session, WeakMap-keyed) build lazily — a unit registered after events flowed, or a read of a session predating the registration, calls `init(session.header)` and folds the in-memory log on first touch.
...
- **Initialization follows the event source.** Live lazy and event-driven builds pass `session.header`; detached restore callers pass the immutable header returned with the same persisted event read. The registry rejects a `seedLength` beyond the observed log before folding. A unit may derive its boundary with `header.seedLength ?? 0`, but must not fetch Session or process state behind the registry.
