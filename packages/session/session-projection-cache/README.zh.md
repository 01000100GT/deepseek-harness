...
持久投影缓存（`ctx.sessionProjectionCache`）：把每个投影单元的状态保存为检查点，在 `per-record` 布局的 `session_projcache` 存储域上为每个会话存一份带版本戳的文档（内置 json 后端把每个会话的记录落在 `<root>/session_projcache/sessions/<id>.json`）。缓存绝不咨询持久化层。设计权威：[session-projection RFC](../../../.agents/notes/proposed/architecture/2026-07-27-session-projection-and-command-log.md)（persisted projection cache 一节）。
