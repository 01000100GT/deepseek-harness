...
- **派生索引，不是源存储。** FTS 行存放在专用可丢弃数据库中；本包从不直接打开持久化产物，但其继承的检查可能要求持久化服务迁移受支持的历史 Session。
