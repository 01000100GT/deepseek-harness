...
`SessionCorpus` 通过 fiber 绑定可选的 `ctx.sessionPersistence`，并实时优先解析每次读取：已知实时目标直接快照，不查询持久化；否则先列出会话、执行检查，并在克隆前重新检查是否出现实时挂载。已经是当前格式的检查只在内存中保留恢复；受支持的历史正文读取可能先发布迁移与修复。列表与加载观察之间会断言 header 兼容性。批量标题读取执行一次元数据列表与有界并发检查，把逐会话失败隔离，而取消会拒绝整个观测批次；已经为其他观察方接纳的共享持久化加载或迁移仍可能继续。
