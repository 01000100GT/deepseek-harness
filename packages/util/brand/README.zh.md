...
品牌使 `SessionId` 和 `ToolCallId` 这样结构相同的字符串在类型层面不可互换，尽管两者在运行时都是普通 `string`。
...
包为自己拥有的 id 添加品牌：`ToolCallId` 位于 `dsh-llm`，共享的 agent/会话 `SessionId` 位于 `dsh-session`，`JobId` 位于 `dsh-jobs`。为可能被混淆的跨包 id 添加品牌，但无需为每个字符串都添加。
