  ...
  private projectionsFor(
    ...
    try {
      ...
      const snapshot = source.kind === 'attached'
        ? source.session.seq - 1 === throughSeq
          ? registry.snapshot(source.session)
          : registry.restore({}, events, 0).snapshot
        // The cache owns the cold-read recipe (seed from the cached
        // checkpoint, skip the cached prefix's applies, write the refreshed
        // checkpoint back); without the cache the registry restores from
        // init over the full log.
        : this.ctx.get('sessionProjectionCache')?.coldSnapshot(source.header, events)
          ?? registry.restore({}, events, 0).snapshot
      return {
