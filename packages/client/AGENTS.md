...
### Viewport unit contract

CSS Modules in any client or extension UI plugin MUST size elements against `var(--app-height, 100dvh)` / `var(--app-width, 100dvw)` instead of any numeric layout-viewport unit (`50vh`, `100vw`, `52vh`, `100svh`, … all forbidden). The web shell writes both custom properties from `visualViewport.{height,width}` in `packages/client/web/src/viewport.ts` (`installStableViewport`, run at the top of `AppWebEntry.run()` so the first paint already has stable values). Layout-viewport units re-introduce the jump the shell fix prevents: when the browser reveals or collapses its URL bar (mobile) or the soft keyboard opens, every element sized by them visibly shifts. Bare `dvh`/`dvw` is allowed because the dynamic units track the same events natively; the contract only forbids the layout-viewport forms. Fractions spell as `calc(var(--app-height, 100dvh) * 0.6)`. `--app-width` is consumed only by popover/panel CSS: the html/body/#root mount chain stays width-unpinned, because during pinch-zoom `visualViewport.width` runs narrower than the layout viewport and clamping the document to it would reflow everything. `verify-client-viewport-units` (`pnpm run hygiene` aggregate) enforces the whole set and reports every offender with file:line:column.
