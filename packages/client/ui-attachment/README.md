...
This package renders everything the conversation UI shows about attachments: one ordered draft rail under the composer, a full-viewport drop invitation, durable images in Chat and Trajectory, and a lightbox for the original image. It is a pure presentation layer — attachment data, upload state, image loading, and callbacks come from the conversation package through declared slots. Choose it for the DeepSeek Chat-style attachment experience.
