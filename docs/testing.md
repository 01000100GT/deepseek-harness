...
Session fixtures keep headers and payloads but omit body sequence/time envelopes. Replay synthesizes them. A scenario may retain older generations for migration coverage, but replay, record, and refresh select the numerically highest file for each parent/child role; current v1 writer output uses `.v1`, while committed released-v0 inputs remain suffixless. Fixtures use canonical packed rows; [the migrator](../scripts/migrate-packed-session-fixtures.ts) rewrites old row layouts.
