...
Every non-trivial model-, protocol-, or human-visible change adds or updates a keyless recorded-session scenario in the same PR; package, e2e, mock-only, and rationale evidence does not replace the assembled transcript. ACP protocol cases live under `snapshots/acp/`, transport-neutral recordings under `snapshots/session/`, and result-only expected output with its owner under `tests/goldens/`; [`dsh-session-snapshot`](../packages/test-support/session-snapshot/README.md) owns the shared rules and adapters. Agent-loop, session-lifecycle, and `SessionEventMap` changes update both SDK projections: `snapshots/sdk/` owns TypeScript, while required Python-runtime CI owns `scripts/snapshots/python-sdk-single-exe/`. New capability seams and lifecycle or transcript variants name every required tier at plan time.
