...
Session fixtures keep headers and payloads but omit body sequence/time envelopes. Replay synthesizes them. A scenario may retain an explicitly declared v0 or v1 generation for migration coverage; otherwise replay, record, and refresh select the numerically highest file for each parent/child role and the current v2 writer uses `.v2`. Fixtures use canonical packed rows; [the migrator](../scripts/migrate-packed-session-fixtures.ts) rewrites old row layouts.
